    ...
    >
      ...
      <div className="relative h-40 md:h-56 w-full overflow-hidden">
        ...
        <div className="absolute top-3 left-3 md:top-4 md:left-4">
          <span 
            className="px-2.5 py-1 md:px-3.5 md:py-1.5 rounded-lg md:rounded-xl text-[9px] md:text-[10px] font-bold uppercase tracking-wider shadow-lg flex items-center gap-2 backdrop-blur-md border border-white/10"
            style={{ backgroundColor: article.sourceColor + '99', color: 'white' }}
          >
            <span className="text-xs md:text-sm">{article.sourceIcon}</span>
            {article.sourceName}
          </span>
        </div>
      ...
      <div className="p-5 md:p-6 flex flex-col flex-grow">
        <div className="flex items-center gap-2 text-[10px] md:text-[11px] font-medium text-slate-500 mb-3 md:mb-4 uppercase tracking-widest">
          <Clock size={12} className="text-indigo-400" />
          {formattedDate}
        </div>
        
        <h3 className="text-lg md:text-xl font-bold mb-3 md:mb-4 line-clamp-2 leading-[1.4] font-heading group-hover:text-indigo-300 transition-colors">
          {article.title}
        </h3>
        
        <p className="text-sm md:text-[15px] text-slate-400 line-clamp-2 md:line-clamp-3 mb-4 md:mb-6 flex-grow leading-relaxed font-light">
          {article.description}
        </p>

        <div className="mt-auto flex items-center justify-between pt-4 md:pt-5 border-t border-white/5">
          <span className="text-xs md:text-sm font-semibold text-indigo-400 flex items-center gap-2 group-hover:gap-3 transition-all">
